...
export default function Home() {
  ...
  return <main>
    ...
    <section className="intro" id="top"><p className="eyebrow">MODEL EVALUATION / 2026</p><h1>Five models.<br/><em>Matched seeds.</em></h1><div className="intro-copy"><p>A controlled visual study of how successive LoRA training strategies move Krea 2 toward a Swiss poster language.</p><div className="counter"><strong>100</strong><span>outputs</span></div><div className="counter"><strong>58</strong><span>training images</span></div></div></section>
    ...
    <section className="method" id="method"><div><p className="eyebrow">V3 TRAINING STRATEGY</p><h2>Teach the system,<br/>not the subject.</h2></div><div className="spec-grid">{[['58','captioned images'],['500','optimization steps'],['1e-4','learning rate'],['16 / 16','LoRA rank / alpha'],['768 px','training resolution'],['4','effective batch']].map(([value,label])=><div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div><p className="method-copy">V3 replaces a single generic instance prompt with one caption per image. Every caption begins with <code>swissposter style</code>, then identifies the visible grid, typography, palette, geometry, imagery, and print character. Lower rank and learning rate are intended to transfer the visual system while preserving Krea’s prompt responsiveness.</p></section>
    <section className="training" id="training"><div className="section-head"><div><p className="eyebrow">SOURCE MATERIAL</p><h2>The complete training set</h2></div><p>58 images spanning geometric systems, typographic grids, photographic collage, product and travel advertising, editorial layouts, and mid-century illustration.</p></div><div className="training-grid">{trainingFiles.map((file,i)=><figure key={file}><a href={`/training/${file}`} target="_blank"><img src={`/training/${file}`} alt={`Training image ${i+1}`} loading="lazy"/></a><figcaption><span>{String(i+1).padStart(3,'0')}</span><a href="/captions.csv" download>caption <ChevronDown size={12}/></a></figcaption></figure>)}</div></section>
    <footer><b>K2 / SWISS POSTER STUDY</b><span>Base + four LoRA experiments</span><a href="#top">Back to top ↑</a></footer>
  </main>;
}
