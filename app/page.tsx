...
export default function Home() {
  ...
  const [activeStrategy,setActiveStrategy]=useState(4);
  ...
  return <main>
    ...
    <section className="workspace" id="compare">
      ...
      <TooltipProvider delay={140}><div className={`comparison ${mode}`} style={{'--columns':selected.length} as React.CSSProperties}><div className="model-heads"><span>SEED</span>{selected.map(m=><div className="model-head" key={m.folder}><div><b>{m.label}</b><small>{m.note}</small></div><RunParameters model={m}/></div>)}</div>{[0,1,2,3].map(seed=><div className="seed-row" key={seed}><div className="seed-label"><span>0{seed}</span><small>seed</small></div>{selected.map(model=><figure key={model.folder}><a href={`/models/${model.folder}/prompt_${activePrompt}_seed_${seed}.jpg`} target="_blank"><img src={`/models/${model.folder}/prompt_${activePrompt}_seed_${seed}.jpg`} alt={`${prompts[activePrompt]}, ${model.label}, seed ${seed}`}/></a><figcaption><b>{model.label}</b><span>Seed {seed}</span></figcaption></figure>)}</div>)}</div></TooltipProvider>
    </section>
